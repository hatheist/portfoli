...
import {captionWordCaps, portfolioSections, showCaptions, sizes} from '@/options'
...
export async function getStaticPaths() {
    ...
    // Filter out portfolio sections that should be handled by dedicated pages
    const galleryPaths = files
        .filter(file => !portfolioSections.includes(file))
        .map((file) => ({
            params: {
...
export async function getStaticProps(ctx: GetStaticPropsContext) {
    ...
    // Check if this is a portfolio section that should be redirected
    if (portfolioSections.includes(slug as string)) {
        // Find the corresponding section URL
        const galleries = galleryList();
        const gallery = galleries.find(g => g.key === slug);
